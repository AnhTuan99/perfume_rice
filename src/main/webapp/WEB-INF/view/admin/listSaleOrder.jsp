					...
					<div class="panel-body">
						...
						<div class="table-responsive">
							<table class="table table-striped table-hover">
								...
								<tbody id="myTable">
								<c:forEach var="s" items="${saleOrderService }" varStatus="loop">
									<tr class="tr-shadow">
										<th scope="row">${loop.index + 1}</th>
										<td style="font-size: 16px;">${s.customer_name}</td>
										<td>${s.customer_address}</td>
										<td>${s.customer_phone}</td>
										<td>${s.cutomer_email}</td>
										<td>
											<fmt:setLocale value="vi_VN" scope="session" />
											<fmt:formatNumber value="${s.total}" type="currency" />
										</td>
										<td>${s.createdDate}</td>
										<td class="text-center">
											<c:choose>
												...
												<c:when test="${s.state == 2}">
													<span class="badge" style="background: green;">Nhận hàng</span>
												</c:when>
												<c:otherwise>
													<span class="badge" style="background: #ffa944;">Hủy</span>
												</c:otherwise>
											</c:choose>
										</td>
										<td class="text-right">
											<a class="btn btn-xs btn-default add-tooltip"
											   data-toggle="tooltip" href="/admin/orderDetail/${s.id}" data-original-title="Edit"
											   data-container="body">
