					...
					<div class="panel-body">
						...
						<div class="table-responsive">
							<table class="table table-striped table-hover">
								...
								<tbody>
								<c:forEach var="l" items="${listSaleOrderProducts }" varStatus="loop">
									<tr class="tr-shadow">
										<th scope="row">${loop.index + 1}</th>
										<td style="font-size: 16px;">${l.saleorder_products.products.title}</td>
										<td><img style="width: 70px; height: 90px;" class="card-img-top" src="/upload/${l.saleorder_products.products.avatar}" alt=""></td>
										<td>${l.saleorder_products.quality}</td>
										<td>
											<fmt:setLocale value="vi_VN" scope="session" />
											<fmt:formatNumber value="${l.saleorder_products.products.price_sale}" type="currency" />
										</td>
										<td>
											<fmt:setLocale value="vi_VN" scope="session" />
											<fmt:formatNumber value="${l.thanhTien}" type="currency" />
										</td>
								...
								<tr class="tr-shadow">
									...
									<td style="font-size: 18px; font-weight: bold;">
										<span class="badge" style="font-size: 18px; font-weight: bold; margin-right: 10px;">
											<fmt:setLocale value="vi_VN" scope="session" />
											<fmt:formatNumber value="${saleorder.total}" type="currency" />
										</span>

									</td>
								</tr>
								<tr>
									<td><button class="btn btn-success ">Xuất hóa đơn</button></td>
									<td><button class="btn btn-primary" ">In hóa đơn</button></td>
									<td></td>
									<td>
										<c:if test="${saleorder.state == 0}">
										<a style="margin-right: 30px;" class="btn btn-danger"
										   href="${pageContext.request.contextPath}/admin/confirmSaleOrder/${saleorder.id}?state=3">Hủy</a>
									...
									<td>
										...
										<c:if test="${saleorder.state == 0}">
											...
											<a class="btn btn-primary"
											   href="${pageContext.request.contextPath}/admin/confirmSaleOrder/${saleorder.id}?state=1">Xác nhận</a>
										</c:if>
										<c:if test="${saleorder.state == 1}">
											<a class="btn btn-info"
											   href="${pageContext.request.contextPath}/admin/confirmSaleOrder/${saleorder.id}?state=2">Nhận hàng</a>

										</c:if>
									</td>
									<td>
										<a href="/admin/listSaleOrder" class="btn btn-secondary active"
